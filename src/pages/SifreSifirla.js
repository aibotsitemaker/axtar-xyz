    ...
    <div className="min-h-[80vh] flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
          ...
          {gonderilib ? (
            <div className="text-center py-4">
              ...
              <p className="text-sm text-gray-500 leading-relaxed mb-6">
                <strong>{email}</strong> ünvanına şifrə sıfırlama linki göndərildi. E-poçtunuzu yoxlayın.
              </p>
            ...
            <>
              ...
              <p className="text-sm text-gray-500 mb-5">E-poçt ünvanınızı daxil edin, sıfırlama linki göndərəcəyik.</p>
              ...
              <div className="mb-4">
                ...
                <input className="input" type="email" placeholder="email@mail.com" value={email} onChange={e => setEmail(e.target.value)} onKeyDown={e => e.key === 'Enter' && gonder()} autoFocus />
              </div>
              <button onClick={gonder} disabled={loading} className="w-full btn-primary justify-center py-3">
                {loading ? <><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Göndərilir...</> : '📧 Link göndər'}
              </button>
            </>
          )}
        </div>
        ...
        <p className="text-center text-sm text-gray-500 mt-4">
          Yadınıza düşdü? <Link to="/giris" className="text-primary-600 font-medium hover:underline">Daxil olun</Link>
        </p>
      </div>
    </div>
  )
