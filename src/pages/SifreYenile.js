...
export default function SifreYenile() {
  ...
  useEffect(() => {
    // Supabase hash-dəki token-i avtomatik işləyir
    supabase.auth.onAuthStateChange((event, session) => {
      if (event === 'PASSWORD_RECOVERY') {
        setReady(true)
      } else if (event === 'SIGNED_IN' && session) {
        // Token artıq işləyib, şifrə dəyişdirməyə hazırıq
        setReady(true)
      }
    })
  ...
  const sifreDeyis = async () => {
    ...
    if (form.yeni.length < 6) { setError('Şifrə minimum 6 simvol olmalıdır'); return }
  ...
  const gucReng = ['', 'bg-red-400', 'bg-orange-400', 'bg-yellow-400', 'bg-green-400'][guc()]
  const gucMetn = ['', 'Zəif', 'Orta', 'Yaxşı', 'Güclü'][guc()]
    ...
    return (
      <div className="min-h-[80vh] flex items-center justify-center px-4">
        <div className="text-center max-w-sm">
          ...
          <h2 className="text-xl font-bold text-gray-800 mb-2">Şifrə uğurla dəyişdirildi!</h2>
          <p className="text-gray-500 text-sm mb-6">3 saniyə sonra profilinizə yönləndiriləcəksiniz...</p>
          <Link to="/dashboard" className="btn-primary justify-center py-3 px-8">Profilə keç</Link>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-[80vh] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-500 text-sm">Token yoxlanılır...</p>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-[80vh] flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        ...
        <div className="card p-6 shadow-sm">
          <div className="bg-blue-50 border border-blue-100 text-blue-700 rounded-xl px-4 py-3 text-sm mb-5">
            🔐 Hesabınız təsdiqləndi. İndi yeni şifrə təyin edə bilərsiniz.
          </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  type={showPass.yeni ? 'text' : 'password'}
                  placeholder="Minimum 6 simvol"
                  value={form.yeni}
                  onChange={e => setForm(p => ({ ...p, yeni: e.target.value }))}
                  autoFocus
                />
              ...
              {form.yeni && (
                <div className="mt-2">
                  <div className="flex gap-1">
                    {[1,2,3,4].map(i => (
                      <div key={i} className={`h-1 flex-1 rounded-full transition-all ${i <= guc() ? gucReng : 'bg-gray-100'}`} />
                    ))}
                  </div>
                  <p className="text-xs text-gray-400 mt-1">{gucMetn}</p>
                </div>
              )}
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  className="input pr-10"
                  type={showPass.yeni2 ? 'text' : 'password'}
                  placeholder="Şifrəni təkrar daxil edin"
                  value={form.yeni2}
                  onChange={e => setForm(p => ({ ...p, yeni2: e.target.value }))}
                  onKeyDown={e => e.key === 'Enter' && !loading && sifreDeyis()}
                />
              ...
              {form.yeni2 && form.yeni !== form.yeni2 && (
                <p className="text-xs text-red-500 mt-1">Şifrələr uyğun gəlmir</p>
              )}
              {form.yeni2 && form.yeni === form.yeni2 && form.yeni.length >= 6 && (
                <p className="text-xs text-green-500 mt-1">✓ Şifrələr uyğundur</p>
              )}
            </div>
          </div>
          ...
          <button
            onClick={sifreDeyis}
            disabled={loading || !form.yeni || !form.yeni2}
          ...
          >
            {loading
              ? <><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Dəyişdirilir...</>
              : '🔐 Şifrəni dəyiş'}
          </button>
        </div>
      </div>
